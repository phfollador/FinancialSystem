    ...
    public class VoucherHandler(AppDbContext context) : IVoucherHandler
    {
        public async Task<Response<Voucher?>> GetByNumberAsync(GetVoucherByNumberRequest request)
        {
            try
            {
                ...
                return voucher is null ? new Response<Voucher?>(null, 404, "Voucher nao encontrado") : new Response<Voucher?>(voucher);
            }
            catch
            {
                return new Response<Voucher?>(null, 500, "Nao foi possivel recuperar o voucher");
            }
        }
    }
}
